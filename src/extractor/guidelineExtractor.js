 ...
 *    - sectionCreated 플래그로 단락당 섹션 생성 1회 제한
 *    - bookmarkStarts.length === 0 → !sectionCreated 로 변경
 * 2. [Fix] createFallbackGuideline: 내용 없는 섹션 null 반환, 호출부 null 체크
 *
 * 변경사항 (v4.4 → v4.4.1):
 * 3. [Fix] convertToGuideline catch 블록: LLM 호출 실패 시 빈 문자열 전달 버그 수정
 *    - content, ruleText를 try 바깥에 선언하여 catch에서 재사용
 *    - LLM 네트워크 에러 시에도 섹션 내용 기반 fallback 규칙 생성 보장
 *
...
export class GuidelineExtractor {
  constructor() {
    ...
    this.guidelines = [];
    this.contextRules = [];
    this.currentFileInfo = { filename: null, prefix: 'GEN' };
  }

  async initialize() {
    if (this.initialized) return;
    logger.info('🚀 가이드라인 추출기 V4.4.1 초기화 중...');
  ...
  async extractAll() {
    ...
    if (files.length === 0) { logger.warn(`docx 파일 없음: ${inputDir}`); return { rules: [], files: 0 }; }
    logger.info(`${files.length}개 docx 파일 발견`);
    const prefixMap = this.generatePrefixMap(files);
    logger.info('📌 파일별 Prefix 할당:');
    for (const [filePath, prefix] of prefixMap.entries()) { logger.info(`   - ${path.basename(filePath)} → ${prefix}`); }
    const allRules = [];
    const sources = [];
    for (const filePath of files) {
      ...
      sources.push({ filename, prefix, rulesCount: result.guidelines.length, contextRulesCount: result.contextRules?.length || 0, extractedAt: fileStartTime.toISOString() });
    }
    ...
    await writeJsonFile(outputPath, { metadata: { source: 'guideline', extractedAt: new Date().toISOString(), version: '4.4.1', totalCount: taggedRules.length, filesProcessed: files.length, sources }, rules: taggedRules });
    logger.info(`✅ ${taggedRules.length}개 룰 저장 완료: ${outputPath}`);
    return { rules: taggedRules, files: files.length, sources, outputPath };
  }
  ...
  generatePrefix(filename) {
    ...
    for (const { keywords, prefix } of mappings) { if (keywords.some(kw => name.includes(kw))) return prefix; }
    return this.extractPrefixFromName(name) || 'GEN';
  }

  extractPrefixFromName(name) {
    const parts = name.split(/[_\-\s]+/);
    if (parts.length >= 2) {
      const prefix = parts.slice(0, 4).map(p => p.charAt(0).toUpperCase()).join('');
  ...
  ensureUniquePrefix(prefix, usedPrefixes) {
    ...
    let counter = 2;
    while (usedPrefixes.has(`${prefix}${counter}`)) counter++;
  ...
  async extractFromDOCX(docxPath) {
    logger.info('📘 DOCX 파싱 시작 (V4.4.1)...');
    try {
      const buffer = await fs.readFile(docxPath);
      this.docxZip = await JSZip.loadAsync(buffer);
      const documentXml = await this.docxZip.file('word/document.xml').async('string');
      const doc = await parseStringPromise(documentXml, { preserveChildrenOrder: true, explicitChildren: true, charsAsChildren: false });
      ...
      const sectionsWithTables = sections.filter(s => s.contentElements.some(e => e.type === 'table'));
      const totalTables = sections.reduce((sum, s) => sum + s.contentElements.filter(e => e.type === 'table').length, 0);
      ...
      if (dupes.length > 0) logger.warn(`⚠️ 중복 섹션 번호 감지: ${[...new Set(dupes)].join(', ')}`);
      this.contextRules = contextSections.map(ctx => ({ ruleId: `ctx.${ctx.contextType}`, title: ctx.title, sectionNumber: ctx.sectionNumber, level: ctx.level, content: this.extractSectionTextOnly(ctx), appliesTo: ctx.appliesTo, contextType: ctx.contextType }));
      ...
      return { contextRules: this.contextRules, guidelines: this.guidelines };
    } catch (error) {
  ...
  async loadImageRelations() {
    try {
      ...
      for (const rel of relationships) {
        ...
        const type = rel.$['Type'];
        if (type && type.includes('image')) this.imageRelations.set(id, target);
      }
    ...
    for (const child of children) {
      ...
      if (hyperlinks.length === 0) {
        if (tocStarted) {
          ...
          if (bookmarks.length > 0) { tocEnded = true; break; }
        }
        continue;
      }
      for (const hyperlink of hyperlinks) {
        ...
        if (!anchor) continue;
        if (anchor.startsWith('_Toc')) tocStarted = true;
        ...
        this.tableOfContents.set(anchor, { level, title: title.trim(), anchor });
      }
    }
  }
  ...
  async extractSectionsByBookmarks(body) {
    ...
    for (const { type, element } of orderedElements) {
      if (type === 'w:p') {
        ...
        // ✅ [Fix v4.4] 단락당 첫 번째 TOC 매칭 북마크만 처리 (sectionCreated 플래그)
        let sectionCreated = false;
        for (const bookmark of bookmarkStarts) {
          if (sectionCreated) break;
          ...
          if (tocEntry) {
            ...
            if (currentSection && this.isValidSection(currentSection)) sections.push(currentSection);
            currentSection = { level: tocEntry.level, sectionNumber: this.inferSectionNumber(tocEntry.title), title: tocEntry.title, anchor: bookmarkName, contentElements: [], isContext: false, contextType: null, appliesTo: null };
            const contextInfo = this.identifyContextSection(currentSection);
            if (contextInfo) { currentSection.isContext = true; currentSection.contextType = contextInfo.contextType; currentSection.appliesTo = contextInfo.appliesTo; }
            sectionCreated = true;
          }
        }
        if (skipUntilTocEnd) continue;

        // ✅ [Fix v4.4] !sectionCreated 로 변경 (TOC 미매칭 북마크 단락 내용 누락 방지)
  ...
  getOrderedBodyElements(body) {
    ...
    if (body.$$) {
      for (const child of body.$$) {
        const tagName = child['#name'];
        if (tagName === 'w:p' || tagName === 'w:tbl') elements.push({ type: tagName, element: child });
      }
    ...
    for (const [key, value] of Object.entries(body)) {
      if ((key === 'w:p' || key === 'w:tbl') && Array.isArray(value)) {
        for (const element of value) elements.push({ type: key, element });
      }
    }
    return elements;
  }
  ...
  findBookmarkStarts(element) {
    const bookmarks = [];
    if (element.$$) { for (const child of element.$$) { if (child['#name'] === 'w:bookmarkStart') bookmarks.push(child); } }
    if (bookmarks.length === 0 && element['w:bookmarkStart']) bookmarks.push(...element['w:bookmarkStart']);
    return bookmarks;
  }

  findChildrenByName(element, name) {
    const children = [];
    if (element.$$) { for (const child of element.$$) { if (child['#name'] === name) children.push(child); } }
    if (children.length === 0 && element[name]) {
      if (Array.isArray(element[name])) children.push(...element[name]);
      else children.push(element[name]);
    }
    return children;
  }

  findChildByName(element, name) {
    if (element.$$) { for (const child of element.$$) { if (child['#name'] === name) return child; } }
    if (element[name]) return Array.isArray(element[name]) ? element[name][0] : element[name];
    return null;
  }
  ...
  extractTextFromElement(element) {
    ...
    const extractRecursive = (el) => {
      if (el['#name'] === 'w:t' && el._) texts.push(el._);
      if (el.$$) { for (const child of el.$$) extractRecursive(child); }
    };
  ...
  extractTextFromParagraph(para) { return this.extractTextFromElement(para); }
  ...
  identifyContextSection(section) {
    ...
    let appliesTo = 'all';
    if (section.level === 2) { const l1Number = section.sectionNumber.split('.')[0]; appliesTo = `section_${l1Number}`; }
  ...
  extractSectionTextOnly(section) {
    ...
    for (const item of section.contentElements) {
      if (item.type === 'paragraph') { const text = this.extractTextFromParagraph(item.element); if (text) textLines.push(text); }
    }
  ...
  extractTableData(tableElement) {
    ...
    if (rows.length === 0) return { type: 'empty', content: '', markdown: '', rows: 0, cols: 0 };
    const tableData = [];
    for (const row of rows) {
      ...
      for (const cell of cells) {
        ...
        const cellTexts = [];
        for (const para of cellParas) { const text = this.extractTextFromParagraph(para); if (text) cellTexts.push(text); }
        rowData.push({ text: cellTexts.join(' '), gridSpan: 1, vMerge: null });
      }
      tableData.push(rowData);
    }
    if (tableData.length === 1 && tableData[0].length === 1) return { type: 'textbox', content: tableData[0][0].text, markdown: '', rows: 1, cols: 1 };
    const markdown = this.convertTableToMarkdown(tableData);
    return { type: 'table', rows: tableData.length, cols: tableData[0]?.length || 0, content: '', markdown };
  }
  ...
  convertTableToMarkdown(tableData) {
    ...
    for (let i = 1; i < tableData.length; i++) { lines.push('| ' + tableData[i].map(cell => cell.text || '').join(' | ') + ' |'); }
    return lines.join('\n');
  }
  ...
  async extractSectionContent(section) {
    const content = { text: '', tables: [], images: [] };
    const textLines = [];
    for (const item of section.contentElements) {
      if (item.type === 'paragraph') { const text = this.extractTextFromParagraph(item.element); if (text) textLines.push(text); }
        ...
        if (table.type === 'textbox') textLines.push(`\n[텍스트박스] ${table.content}\n`);
        else textLines.push('\n' + table.markdown + '\n');
      }
    }
  ...
  async convertToGuideline(section) {
    // ✅ [Fix v4.4.1] content, ruleText를 try 바깥에 선언 → catch에서 재사용 가능
    // 기존: catch에서 빈 문자열('') 전달 → effectiveText < 30 → null → 섹션 규칙 누락
    // 수정: LLM 에러가 발생해도 이미 계산된 섹션 내용으로 fallback 규칙 생성
    let content = null;
    let ruleText = '';

    try {
      content = await this.extractSectionContent(section);
      ruleText = `${section.sectionNumber} ${section.title}\n\n${content.text}`;
      ...
      // ✅ [Fix v4.4.1] 이미 계산된 content/ruleText 재사용 (null이면 빈 값으로 대체)
      const fallbackContent = content ?? { text: '', tables: [], images: [] };
      const fallbackRuleText = ruleText || '';
      const guideline = this.createFallbackGuideline(section, fallbackContent, fallbackRuleText);
      if (guideline) this.guidelines.push(guideline);
    }
  }
...
JSON만 출력하세요.`;
  ...
  normalizeGuideline(result, section, content, ruleText) {
    const validCategories = ['resource_management', 'security', 'exception_handling', 'performance', 'architecture', 'code_style', 'naming_convention', 'documentation', 'general'];
    const category = validCategories.includes(result.category?.toLowerCase()) ? result.category.toLowerCase() : this.inferCategory(section.title, ruleText);
    const validSeverities = ['CRITICAL', 'HIGH', 'MEDIUM', 'LOW'];
    const severity = validSeverities.includes(result.severity?.toUpperCase()) ? result.severity.toUpperCase() : this.inferSeverity(section.title, ruleText);
    const categoryPrefix = { 'resource_management': 'RES', 'security': 'SEC', 'exception_handling': 'ERR', 'performance': 'PERF', 'architecture': 'ARCH', 'code_style': 'STY', 'naming_convention': 'NAM', 'documentation': 'DOC', 'general': 'GEN' };
    ...
    return {
      ruleId, sectionNumber: section.sectionNumber, title: result.title || section.title, level: section.level, category, severity,
      description: result.description || ruleText.substring(0, 500), message: result.message || result.title || section.title, suggestion: result.suggestion || '',
      source: `guideline:${section.sectionNumber}`, sourceFile: this.currentFileInfo.filename, sourcePrefix: docPrefix, isActive: true,
      problematicCode: result.badExample || null, fixedCode: result.goodExample || null,
      keywords: result.keywords || this.extractKeywordsFromText(section.title, ruleText),
      hasTables: content.tables.length > 0, hasImages: content.images?.length > 0, tables: content.tables,
      metadata: { createdAt: new Date().toISOString(), source: `${section.sectionNumber} ${section.title}`, sourceFile: this.currentFileInfo.filename, version: '4.4.1' }
    };
  }

  // ✅ [Fix v4.4] 내용이 없는 섹션은 null 반환
  createFallbackGuideline(section, content, ruleText) {
    ...
    const ruleId = `${docPrefix}.${catPrefix}.${section.sectionNumber.replace(/\./g, '_')}`;
    return {
      ruleId, sectionNumber: section.sectionNumber, title: section.title, level: section.level, category,
      severity: this.inferSeverity(section.title, ruleText || ''), description: ruleText?.substring(0, 500) || section.title,
      message: `${section.title} 규칙을 위반했습니다`, source: `guideline:${section.sectionNumber}`,
      sourceFile: this.currentFileInfo.filename, sourcePrefix: docPrefix, isActive: true,
      keywords: this.extractKeywordsFromText(section.title, ruleText || ''),
      metadata: { createdAt: new Date().toISOString(), source: `${section.sectionNumber} ${section.title}`, sourceFile: this.currentFileInfo.filename, version: '4.4.1', isFallback: true },
      hasTables: content.tables?.length > 0, tables: content.tables || []
    };
  }
  ...
  extractKeywordsFromText(title, content) {
    ...
    const javaKeywords = ['exception', 'try', 'catch', 'finally', 'throw', 'connection', 'stream', 'resource', 'close', 'sql', 'injection', 'security', 'validation', 'controller', 'service', 'repository', 'dao', 'annotation', 'interface', 'abstract', 'class', 'static', 'final', 'synchronized', 'volatile'];
    for (const kw of javaKeywords) { if (text.includes(kw)) keywords.push(kw); }
    return keywords;
  }
  ...
  sortGuidelines() {
    this.guidelines.sort((a, b) => {
      ...
      for (let i = 0; i < Math.max(aParts.length, bParts.length); i++) {
        const aVal = aParts[i] || 0; const bVal = bParts[i] || 0;
...
export function getGuidelineExtractor() {
  if (!instance) instance = new GuidelineExtractor();
  return instance;
}
